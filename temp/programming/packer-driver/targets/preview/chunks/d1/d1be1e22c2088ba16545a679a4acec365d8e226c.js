  ...
  return {
    ...
    execute: function () {
      ...
      _export("Controller", Controller = (_dec = ccclass('Controller'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(_crd && NodeCustom === void 0 ? (_reportPossibleCrUseOfNodeCustom({
        ...
        CreateGroundStart() {
          var i = random();
          var posX = this.groundCurr.GetSize().x / 2 + this.width / 2;

          if (i < 0.5) {
            posX = -this.groundCurr.GetSize().x / 2 - this.width / 2;
          }

          var posY = this.groundCurr.GetPositon().y + this.groundCurr.GetSize().y + 1;
        ...
        StateGood() {
          ...
          var newSizeX = Math.min(500, this.groundNext.GetSize().x * 1.2);
        ...
        StateNormal() {
          ...
          this.groundNext.GetRb().enabled = false;
          this.groundNext.SetIsMoving(false);
          var newSizeX = this.groundNext.GetSize().x - Math.abs(this.groundCurr.GetPositon().x - this.groundNext.GetPositon().x);
          var posCurr = this.groundNext.GetPositon();
          var newPos = new Vec3(posCurr.x + (this.groundCurr.GetPositon().x - this.groundNext.GetPositon().x) / 2, posCurr.y, posCurr.z);
          ...
          var size_Excess = Math.abs(this.groundCurr.GetSize().x - this.groundNext.GetSize().x);
          var pos_Excess = null;
        ...
        onTouchStart(event) {
          ...
          var distance = Math.abs(this.groundCurr.GetPositon().x - this.groundNext.GetPositon().x);
        ...
        CameraFollow() {
          ...
          var posY = Math.max(this.height / 2, this.groundCurr.GetPositon().y + this.height / 2);
        ...
        BackgroundFollow() {
          var posX = this.cameraFollow.position.x - this.width / 2;
          var posY = this.cameraFollow.position.y - this.height / 2;
...
//# sourceMappingURL=d1be1e22c2088ba16545a679a4acec365d8e226c.js.map